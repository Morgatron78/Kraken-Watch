...
| Octoplus | `octoplus.js` — standalone card between Billing and Insights: Octopoints balance (`loyaltyPointLedgers.balanceCarriedForward`), upcoming Saving Sessions / Free Electricity events with reward and a "Joined" marker (`savingSessions`), unused Wheel of Fortune spins (`wheelOfFortuneSpinsAllowed`). Fires an `octoplusAccountInfo` enrolment probe first; if it errors or the account isn't `ENROLLED`, the whole card stays hidden. Each remaining part is its own query so one 401 doesn't sink the rest. Query shapes follow the Home Assistant Octopus Energy integration | **Live**, best-effort side feed (out of the sync-status calc) |
