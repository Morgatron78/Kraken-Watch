...
| EV charging | Rewritten on Octopus's newer SmartFlex API (`devices` → `SmartFlexVehicle` → `chargingSessions`), replacing the old dispatch-only query. Live battery %, target SoC/time, charging power while active, an isSuspended warning when Octopus has disabled device control, and a Windows/Sessions toggle — dispatch-window view (30-min granularity) or whole-session view (SMART/BOOST badges, battery % reached per session), both derived from one query. Week chart stacked by session type (mint=Smart, pink=Boost, matching the same off-peak/peak convention used in the electricity consumption breakdown), tap any day for that day's breakdown. Cost is deliberately not shown — confirmed via diagnostics that Octopus's API returns `cost: null` for every session, so a shown figure would misleadingly read as free rather than unavailable. No fallback to the old dispatch-only path on failure — shows a genuine Unavailable state instead, recovering on the next auto-sync; the old implementation is archived (not shipped) for reference | **Live**, via Kraken GraphQL |
