...
| Billing | Account balance and projected balance, Direct Debit (estimated), Spend this month/Predicted, bill history (last 15 bills, most recent expanded, itemized per-fuel with links to the real bill), a bill-total-over-time chart (last 12 distinct months). Octopoints live in their own **Octoplus** card (next row) | **Live** |
...
- **Octoplus event-type labels** come from a small hand-kept `KIND` map
  (`TURN_DOWN` → "Saving Session", `TURN_UP`/`POWER_UP` → "Free
  electricity", etc.), falling back to a humanized version of the raw
  enum for anything unmapped — no published enum list to check against.
