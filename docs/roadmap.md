...
| **Billing** | Account balance + projected balance, Direct Debit (estimated), spend MTD + predicted, last-15-bills history (itemised per-fuel, links to real bill PDFs), bill-total-over-time chart (12 distinct months). Octopoints surfaced in the Octoplus card (see below). |
